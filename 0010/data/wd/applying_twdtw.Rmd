...
```{r runAnalysisOrUseCache, echo = FALSE, eval = TRUE}
if(exists("twdtw_vignette_usecache") && twdtw_vignette_usecache) {
  cacheFile <- "lucc_MT/assessment.RData"
  cat("Using cached calculations from file", cacheFile)
  load(system.file(cacheFile, package = "dtwSat"))
} else {
  seed <- 1
  cat("Running partitioning and twdtw analysis using seed", seed, "\n")
  set.seed(seed)
  # TODO could be interesting to change splitting of the training dataset
  partitions = splitDataset(field_samples_ts, p=0.1, times=100, freq = 8, formula = y ~ s(x, bs="cc"))

  # TODO parameters of weight function could be interesting to change for readers
  log_fun = logisticWeight(alpha=-0.1, beta=50) 
  twdtw_res = lapply(partitions, function(x){
    # parameters could be interesting to change
    res = twdtwApply(x = x$ts, y = x$patterns, weight.fun = log_fun, n=1)
    twdtwClassify(x = res)
  })
  assessment = twdtwAssess(twdtw_res)
